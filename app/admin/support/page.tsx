...
export default function AdminSupportPage() {
  ...
  const [quickReplying, setQuickReplying] = useState<string | null>(null)
  ...
  const suggestReply = async (overrideAskOnly?: boolean) => {
    if (!selectedTicket || messages.length === 0) return
    const shouldAskOnly = overrideAskOnly !== undefined ? overrideAskOnly : askOnly
    setSuggesting(true)
    try {
  ...
  const quickDelayReply = async (e: React.MouseEvent, ticket: Ticket) => {
    e.stopPropagation()
    if (quickReplying) return
    setQuickReplying(ticket.id)
    try {
      // 1. Fetch ticket details
      const detailRes = await fetch(`/api/support/${ticket.id}?admin=true`)
      if (!detailRes.ok) throw new Error('Failed to load ticket')
      const data = await detailRes.json()
      const ticketMessages = data.messages || []
      const profile = data.profile
      const ticketCheckouts = data.checkouts || []

      if (ticketMessages.length === 0) {
        toast({ title: 'No messages', description: 'This ticket has no messages to reply to', variant: 'destructive' })
        return
      }

      // 2. Generate AI ask-only response
      const suggestRes = await fetch('/api/support/suggest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          subject: ticket.subject,
          messages: ticketMessages.map((m: Message) => ({ is_admin: m.is_admin, message: m.message })),
          profile: profile ? {
            ...
            gender: profile.is_male === true ? 'male' : profile.is_male === false ? 'female' : null,
            memberSince: profile.created_at,
          } : null,
          checkouts: ticketCheckouts.map((c: Checkout) => ({
            product: c.product_description || c.product_type,
            amount: c.total_order,
            date: c.checkout_time,
          })),
      ...
      if (!suggestRes.ok) throw new Error('AI failed')
      const { suggestion } = await suggestRes.json()

      // 3. Send the message
      const sendRes = await fetch(`/api/support/${ticket.id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: suggestion, isAdmin: true }),
      })
      if (!sendRes.ok) throw new Error('Failed to send')

      toast({ title: 'Sent!', description: 'Quick follow-up question sent' })
      fetchTickets()
    } catch {
      toast({ title: 'Error', description: 'Quick reply failed', variant: 'destructive' })
    } finally {
      setQuickReplying(null)
    }
  }
                      ...
                      <TableCell>
                        {!ticket.last_admin_reply && ticket.status !== 'closed' && ticket.status !== 'resolved' && (
                          <Button
                            ...
                            disabled={quickReplying === ticket.id}
                            onClick={(e) => quickDelayReply(e, ticket)}
                          >
                            {quickReplying === ticket.id ? (
                              <Loader2 className="w-3 h-3 animate-spin" />
                            ) : (
                              <><MessageSquare className="w-3 h-3 mr-1" /> Quick Ask</>
                            )}
                          </Button>
                        )}
                      </TableCell>
